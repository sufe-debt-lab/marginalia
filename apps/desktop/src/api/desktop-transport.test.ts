...
it("does not dispatch an already aborted request", async () => {
  ...
  controller.abort();
  await expect(
    desktopPiServerFetch("marginalia://pi-server/workspaces", { signal: controller.signal })
  ).rejects.toMatchObject({ name: "AbortError" });
  expect(window.marginalia.requestPiServer).not.toHaveBeenCalled();
});

it("cancels the main request when the response reader is cancelled", async () => {
  const cancel = vi.fn();
  window.marginalia = {
    getPiServerStatus: vi.fn(),
    restartPiServer: vi.fn(),
    requestPiServer: vi.fn((_input, callback) => {
      callback({ type: "start", status: 200, statusText: "OK", headers: [] });
      return cancel;
    })
  };
  const response = await desktopPiServerFetch("marginalia://pi-server/workspaces");
