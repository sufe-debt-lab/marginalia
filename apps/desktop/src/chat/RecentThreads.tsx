...
import { SectionLabel } from "@/settings/SettingsPrimitives.js";
...
export function RecentThreads({ api }: { api: ApiClient }) {
  ...
  return (
    <div className="mt-[30px]">
      <SectionLabel className="mb-1.5 mt-0">{t("newThread.pickUp")}</SectionLabel>
        ...
        {recent.map((s) => {
          ...
          return (
            <button
              ...
              className="flex items-center gap-3 rounded-[9px] px-2.5 py-2.5 text-left transition-colors motion-fast active:scale-[0.99] [@media(hover:hover)]:hover:bg-accent/70"
            >
