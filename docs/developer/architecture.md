...
- `GET /health` 公开，其余 HTTP route 统一要求每进程 Loopback Access bearer 与 exact Origin；renderer
  通过 main-owned private transport 访问，不持有实际 URL/bearer。该边界不抵御能够读取同用户进程内存或
  控制 Electron main 的恶意软件，随机 loopback 端口本身也不是凭据。
