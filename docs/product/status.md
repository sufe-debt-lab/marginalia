...
| CAP-SKILLS-001    | Skills                        | partial     | 六级 discovery、immutable Catalog、受保护管理 API、revision-pinned runtime，以及带 trigger/workspace guard 的 `$`/`/` picker 已实现 | Settings 启停/预览和 blocked selection 修复未接入；API 不创建、安装或编辑 Skill           | P1-EXTENSIONS-001                       | `apps/pi-server/src/skills/catalog.ts`、`apps/desktop/src/hooks/useSkillCatalog.ts`、`apps/desktop/src/chat/Composer/Composer.tsx` | post-M0                    |
...
| P1-EXTENSIONS-001 | P1       | open        | Skills Composer 选择已接通，Settings 管理与 MCP 仍未接入                        | 2026-07-18    | post-M0                    | `apps/desktop/src/chat/Composer/Composer.tsx`、`apps/desktop/src/settings/SettingsView.tsx`                            |
