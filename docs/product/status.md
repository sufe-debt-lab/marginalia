...
| ----------------- | ----------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| CAP-DESKTOP-001   | Electron shell 与本地 server  | implemented | Electron 启动后拉起随机端口 pi-server，为每个进程注入独立 run capability，renderer 完成健康检查后进入应用  | capability 只保护 run/后续 Skills API；ready 后退出不自动反映到 UI；renderer sandbox 未开启 | P0-SEC-001、P0-SEC-006、P1-RECOVERY-001 | `apps/desktop/electron/main.ts`、`apps/desktop/electron/pi-server-spawner.ts`                                                   | M0-trustworthy-local-alpha |
