...
| CAP-SKILLS-001    | Skills                        | partial     | 六级 discovery、immutable Catalog、受保护管理 API、revision-pinned runtime、Composer picker，以及带 pending 重试守卫的 Settings 启停/预览已实现 | blocked selection 修复未接入；API 不创建、安装、编辑或删除 Skill                          | P1-EXTENSIONS-001                       | `apps/pi-server/src/skills/catalog.ts`、`apps/desktop/src/settings/SkillsPane.tsx`、`apps/desktop/src/chat/Composer/Composer.tsx` | post-M0                    |
...
| P1-EXTENSIONS-001 | P1       | open        | Skills Composer 选择和 Settings 管理已接通；blocked selection 修复与 MCP 未接入 | 2026-07-18    | post-M0                    | `apps/desktop/src/chat/Composer/Composer.tsx`、`apps/desktop/src/settings/SkillsPane.tsx`                              |
