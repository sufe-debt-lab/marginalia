...
| CAP-RUN-001       | Run 生命周期与恢复            | partial     | Run 要求每进程 bearer 与 Origin 检查；状态写入 SQLite，断连时挂起审批会过期                                      | 其他 loopback route 未认证；服务端无同 session single-flight；视图卸载不终止旧 run        | P0-SEC-001、P0-RUN-001、P1-RECOVERY-001 | `apps/pi-server/src/security/capability.ts`、`apps/pi-server/src/app.ts`、`apps/desktop/src/hooks/useStreamingChat.ts`          | M0-trustworthy-local-alpha |
| CAP-SKILLS-001    | Skills                        | partial     | 六级 discovery、immutable Catalog、SQLite preference、受保护管理 API 与 revision-pinned run runtime 已实现       | Desktop Settings/Composer 尚未接入；API 不创建、安装或编辑 Skill                          | P1-EXTENSIONS-001                       | `apps/pi-server/src/skills/catalog.ts`、`apps/pi-server/src/skills/turn-preflight.ts`、`apps/pi-server/src/app.ts`              | post-M0                    |
...
| ----------------- | -------- | ----------- | ------------------------------------------------------------------------------- | ------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| P0-SEC-001        | P0       | open        | Run 已有局部 capability/CORS 防护，其他 loopback route 仍未认证                 | 2026-07-18    | M0-trustworthy-local-alpha | `apps/pi-server/src/security/capability.ts`、`apps/pi-server/src/app.ts`、`apps/desktop/electron/pi-server-spawner.ts` |
...
| P0-SEC-004        | P0       | open        | 缓存 AgentSession 已按 Skill effective revision 重建，但仍忽略后续 model/tools  | 2026-07-18    | M0-trustworthy-local-alpha | `apps/pi-server/src/agent/agent-session-registry.ts`                                                                   |
...
| P1-EXTENSIONS-001 | P1       | open        | Skills runtime 后端已接通但桌面入口不可用；MCP 仍未接入                         | 2026-07-18    | post-M0                    | `apps/pi-server/src/agent/pi-coding-agent-client.ts`、`apps/desktop/src/settings/SettingsView.tsx`                     |
...
| Check                | Result                                                                                       |
| -------------------- | -------------------------------------------------------------------------------------------- |
| `pnpm test`          | 通过；docs 28、chat-core 14、pi-server 261、desktop 330，1 个真实 MiniMax live test 默认跳过 |
| `pnpm typecheck`     | 通过                                                                                         |
| `pnpm lint`          | 通过                                                                                         |
