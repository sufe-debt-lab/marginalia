...
| CAP-SKILLS-001    | Skills                        | partial     | 六级 discovery、immutable Catalog、SQLite preference 与受 capability 保护的 list/state/content 管理 API 已实现   | Desktop Settings/Composer 尚未接入，运行时仍设置 `noSkills: true`；API 不创建、安装或编辑 Skill | P1-EXTENSIONS-001                       | `apps/pi-server/src/skills/catalog.ts`、`apps/pi-server/src/app.ts`、`apps/desktop/src/settings/SettingsView.tsx`               | post-M0                    |
...
| Check                | Result                                                                                       |
| -------------------- | -------------------------------------------------------------------------------------------- |
| `pnpm test`          | 通过；docs 28、chat-core 14、pi-server 243、desktop 330，1 个真实 MiniMax live test 默认跳过 |
| `pnpm typecheck`     | 通过                                                                                         |
| `pnpm lint`          | 通过                                                                                         |
| `pnpm format:check`  | 通过                                                                                         |
| `pnpm build`         | 通过；renderer 和 PDF worker 仍有大 bundle 警告                                              |
| `pnpm verify:visual` | Task 8 未运行；只增加 backend/API 和正式文档，没有 UI-visible change                         |
