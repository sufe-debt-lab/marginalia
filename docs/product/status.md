...
| CAP-SKILLS-001    | Skills                        | partial     | 六级 discovery、immutable Catalog、SQLite preference、受保护管理 API、shadowed winner 409 与 revision-pinned run runtime 已实现 | Desktop Settings/Composer 尚未接入；API 不创建、安装或编辑 Skill                          | P1-EXTENSIONS-001                       | `apps/pi-server/src/skills/catalog.ts`、`apps/pi-server/src/skills/turn-preflight.ts`、`apps/pi-server/src/app.ts`              | post-M0                    |
