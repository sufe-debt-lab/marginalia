...
4. **UI changes → Electron screenshot**: if anything user-visible in `apps/desktop` changed, the change must be verified in the real Electron window:
   ```bash
   pnpm --filter @marginalia/desktop dev
   ```
   Opening the Vite page in a plain browser does **not** satisfy the gate. Capture the key state(s); for i18n-affecting changes capture both English and 中文.
